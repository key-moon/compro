// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/ALDS1_3_A/judge/4886309/C#
using System;
...
public static class P
{
    ...
    {
        Deque<int> stack = new Deque<int>();
        foreach (var item in Console.ReadLine().Split())
        {
            if (item.All(char.IsDigit))
            {
                stack.PushBack(int.Parse(item));
                continue;
            }
            switch (item)
            {
                case "+":
                    stack.PushBack(stack.PopBack() + stack.PopBack());
                    break;
                case "-":
                    stack.PushBack(-stack.PopBack() + stack.PopBack());
                    break;
        ...
        Console.WriteLine(stack.PopBack());
    }
}
