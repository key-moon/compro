// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/DSL_2_B/judge/3417198/C#
using System;
using System.IO;
using System.Linq;
...
class P
{
    static void Main()
    {
        ...
        SegmentTree<int> RSQ = new SegmentTree<int>(nq[0], 0, (x, y) => x + y);
        for (int i = 0; i < nq[1]; i++)
        {
            var a = Console.ReadLine().Split().Select(int.Parse).ToList();
            if (a[0] == 0) RSQ.Operate(a[1] - 1, a[2]);
            if (a[0] == 1) Console.WriteLine(RSQ.Query(a[1] - 1, a[2]));
        }
    }
}
