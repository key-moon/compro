// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/DSL_2_D/judge/3955254/C#
using System;
using System.IO;
using System.Linq;
...
static class P
{
    const uint inf = int.MaxValue;
    static void Main()
    {
        SegmentTree<uint> segTree = new SegmentTree<uint>(NextInt, inf, (x, y) => y == inf ? x : y);
        int q = NextInt;
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < q; i++)
        {
            if (NextInt == 0)
                segTree.Operate(NextInt, NextInt, NextUInt);
            else
