// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/DSL_2_E/judge/3142782/C#
using System;
using System.IO;
using System.Linq;
...
class P
{
    static void Main()
    {
        ...
        LazySegmentTree<int,int> ruq = new LazySegmentTree<int, int>(n, int.MaxValue, 0, (x, y) => (x + y), x => x);
        for (int i = 0; i < q; i++)
        {
            ...
            if (query[0] == 0)
            {
                ruq.Update(query[1] - 1, query[2] - 1, query[3]);
            }
            else
            {
                res.Add(ruq.Query(query[1] - 1));
            }
        }
