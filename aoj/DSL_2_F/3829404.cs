// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/DSL_2_F/judge/3829404/C#
using System;
using System.IO;
using System.Linq;
...
class SegmentTree<DataT, OperatorT>
{
    ...
    {
        get { Propagate(index + LeafCount); return Data[index]; }
        set { Propagate(index += LeafCount); Data[index] = value; Calculate(index, index); }
    }
    ...
    {
        Operators[(ind << 1)] = MergeOperator(Operators[(ind << 1)], Operators[ind]);
        Operators[(ind << 1) | 1] = MergeOperator(Operators[(ind << 1) | 1], Operators[ind]);
