// detail: https://onlinejudge.u-aizu.ac.jp/status/users/keymoon/submissions/1/DSL_2_F/judge/4060414/C#
using System;
using System.IO;
using System.Linq;
...
public static class P
{
    public static void Main()
    {
        int n = NextInt;
        var q = NextInt;
        SegmentTree<int, int> segTree = new SegmentTree<int, int>(n, int.MaxValue, int.MaxValue, Min, (x, y) => y == int.MaxValue ? x : y, (x, y) => y == int.MaxValue ? x : y);
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < q; i++)
        {
            if (NextInt == 0)
                segTree.Update(NextInt, NextInt, NextInt);
            else
