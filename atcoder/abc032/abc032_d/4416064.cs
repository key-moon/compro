// detail: https://atcoder.jp/contests/abc032/submissions/4416064
using System;
using System.Linq;
...
class P
{
    static void Main()
    {
        ...
        Console.WriteLine(vws.Min(x => x[0]) <= 1000 ? Solve3(nw, vws) : vws.Min(x => x[1]) <= 1000 ? Solve2(nw, vws) : Solve1(nw, vws));
    }
    ...
    static long Solve1(List<int> nw, List<List<int>> vws)
    {
        ...
        var fArray = firstHalf.OrderBy(x => x.Key).ToList();
        ...
        for (sptr = sArray.Count - 1; sptr >= 0; sptr--)
        {
            while (fptr < fArray.Count && fArray[fptr].Key + sArray[sptr].Key <= nw[1])
            {
