// detail: https://atcoder.jp/contests/abc145/submissions/23311309
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        var pos = Enumerable.Repeat(0, n).Select(_ => Console.ReadLine().Split().Select(int.Parse).ToArray()).Select(x => new Vector(x[0], x[1])).ToArray();
        var res = Permutations(Enumerable.Range(0, n).ToArray()).Average(item => item.Zip(item.Skip(1), (x, y) => (pos[x] - pos[y]).SqrtLength).Sum());
...
struct Vector
{
    ...
    public static Vector operator -(Vector a, Vector b) => new Vector(a.x - b.x, a.y - b.y);
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector operator *(Vector a, double op) => new Vector(a.x * op, a.y * op);
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector operator /(Vector a, double op) => new Vector(a.x / op, a.y / op);
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
