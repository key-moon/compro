// detail: https://atcoder.jp/contests/abc161/submissions/11533577
using System;
...
public static class P
{
    ...
    {
        var nkc = Console.ReadLine().Split().Select(int.Parse).ToArray();
        var k = nkc[1];
        var c = nkc[2];
        var s = Console.ReadLine();
        var greedyRes = new SegmentTree<int>(s.Length, 0, Max);
        greedyRes[s.Length - 1] = s[s.Length - 1] == 'o' ? 1 : 0;
        for (int i = s.Length - 2; i >= 0; i--)
        {
            if (s[i] == 'x') continue;
            greedyRes[i] = greedyRes.Query(i + c + 1, s.Length - 1) + 1;
        }
        int res = 0;
        int curRes = 0;
        int breakLimit = -1;
        for (int i = 0; i < s.Length; i++)
        {
            if (s[i] == 'o')
            {
                var afterMax = greedyRes.Query(Max(i, breakLimit) + 1, s.Length - 1);
                if (curRes + afterMax < k) { Console.WriteLine(i + 1); }
                if (i > breakLimit) { curRes++; breakLimit = i + c; }
            }
        }
    }
}
