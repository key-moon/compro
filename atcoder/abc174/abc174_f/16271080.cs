// detail: https://atcoder.jp/contests/abc174/submissions/16271080
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        int[] res = new int[q];
        int[] rPos = Enumerable.Repeat(-1, n + 1).ToArray();
        SegmentTree<int> segTree = new SegmentTree<int>(n + 1, 0, (x, y) => x + y);
        var last = 0;
        foreach (var (query, ind) in queries.Select((elem, ind) => (elem, ind)).OrderBy(x => x.elem[1]))
        {
            var l = query[0] - 1;
            var r = query[1] - 1;
            for (int i = last; i <= r; i++)
            {
                if (rPos[c[i]] != -1) segTree[rPos[c[i]]]--;
                segTree[rPos[c[i]] = i]++;
            }
            res[ind] = segTree.Query(l, r);
            last = r + 1;
        }
