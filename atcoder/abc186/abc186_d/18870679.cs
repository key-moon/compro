// detail: https://atcoder.jp/contests/abc186/submissions/18870679
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        var sorted = a.Distinct().OrderBy(x => x).Select((elem, ind) => (elem, ind)).ToArray();
        var dic = sorted.ToDictionary(x => x.elem, x => x.ind);
        SegmentTree<long> sumSegTree = new SegmentTree<long>(n, 0, (x, y) => x + y);
        SegmentTree<int> cntSegTree = new SegmentTree<int>(n, 0, (x, y) => x + y);
        var res = 0L;
        for (int i = 0; i < n; i++)
        {
            var ind = dic[a[i]];
            res += cntSegTree.Query(0, ind - 1) * a[i] - sumSegTree.Query(0, ind - 1);
            res += sumSegTree.Query(ind + 1, n - 1) - cntSegTree.Query(ind + 1, n - 1) * a[i];
            sumSegTree[ind] += a[i];
            cntSegTree[ind]++;
        }
