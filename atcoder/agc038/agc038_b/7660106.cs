// detail: https://atcoder.jp/contests/agc038/submissions/7660106
using System;
using System.IO;
using System.Net;
...
class DisjointSparseTable<T>
{
    ...
    public DisjointSparseTable(T[] data, Func<T, T, T> merge)
    {
        ...
        for (int layer = 1; layer < Height; layer++)
        {
            ...
            {
                i = layerOffset + (block + 1) * blockSize - 1; Table[i] = Table[i - layerOffset]; i--;
                for (; i >= layerOffset + block * blockSize; i--) Table[i] = Merge(Table[i - layerOffset], Table[i + 1]);

                i = layerOffset + (block + 1) * blockSize; Table[i] = Table[i - layerOffset]; i++;
                for (; i < layerOffset + (block + 2) * blockSize; i++) Table[i] = Merge(Table[i - 1], Table[i - layerOffset]);
            }
            if (Size <= (block + 1) * blockSize) continue;
            {
                i = layerOffset + Min((block + 1) * blockSize, Size) - 1; Table[i] = Table[i - layerOffset]; i--;
                for (; i >= layerOffset + block * blockSize; i--) Table[i] = Merge(Table[i - layerOffset], Table[i + 1]);

                i = layerOffset + (block + 1) * blockSize;Table[i] = Table[i - layerOffset]; i++;
                var max = layerOffset + Min((block + 2) * blockSize, Size);
    ...
    {
        int res = 0;
        for (int i = 1 << 4; i > 0; i >>= 1) if (0 < (n >> (res | i))) res |= i;
        return res;
    }

    /*
    static int[] MSBTable = new int[1 << 16];
    static DisjointSparseTable()
    {
        for (int i = 1; i <= 16; i++)
        {
    ...
    public static int GetMSB(int n) => MSBTable[n];*/
}
