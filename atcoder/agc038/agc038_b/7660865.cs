// detail: https://atcoder.jp/contests/agc038/submissions/7660865
using System;
using System.IO;
using System.Net;
    ...
    public DisjointSparseTable(T[] data, Func<T, T, T> merge)
    {
        ...
        for (int layer = 1; layer < Height; layer++)
        {
            ...
            for (; block + 2 <= Size >> layer; block += 2)
            {
                ...
                bound = layerOffset + (block << layer);
                for (; i >= bound; i--) Table[i] = Merge(Table[i - layerOffset], Table[i + 1]);

                i = layerOffset + ((block | 1) << layer); Table[i] = Table[i - layerOffset]; i++;
                bound = layerOffset + ((block + 2) << layer);
                for (; i < bound; i++) Table[i] = Merge(Table[i - 1], Table[i - layerOffset]);
            }
            if (Size <= ((block | 1) << layer)) continue;
            {
                i = layerOffset + Math.Min((block | 1) << layer, Size) - 1; Table[i] = Table[i - layerOffset]; i--;
                bound = layerOffset + (block << layer);
                for (; i >= bound; i--) Table[i] = Merge(Table[i - layerOffset], Table[i + 1]);

                i = layerOffset + ((block | 1) << layer); Table[i] = Table[i - layerOffset]; i++;
                bound = layerOffset + Size;
                for (; i < bound; i++) Table[i] = Merge(Table[i - 1], Table[i - layerOffset]);
            }
        }
    }
