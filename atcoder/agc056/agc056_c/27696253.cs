// detail: https://atcoder.jp/contests/agc056/submissions/27696253
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        List<(int, int)>[] graph = Enumerable.Repeat(0, n + 1).Select(_ => new List<(int, int)>()).ToArray();
        for (int i = 0; i < n; i++)
        {
            graph[i].Add((i + 1, 1));
            graph[i + 1].Add((i, 1));
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            graph[st[0]].Add((st[1] + 1, 0));
            graph[st[1] + 1].Add((st[0], 0));
        }

        int[] costs = Enumerable.Repeat(int.MaxValue / 2, n + 1).ToArray();
        PriorityQueue<(int, int), int> pqueue = new PriorityQueue<(int, int), int>(x => x.Item2);
        pqueue.Push((0, 0));
        while (pqueue.Count != 0)
        {
            var (elem, cost) = pqueue.Pop();
            if (costs[elem] <= cost) continue;
            costs[elem] = cost;
            foreach (var (to, dist) in graph[elem])
            {
                var nxtCost = cost + dist;
                if (costs[to] <= nxtCost) continue;
                pqueue.Push((to, nxtCost));
            }
        }
        ...
        for (int i = 0; i < n; i++)
        {
            var d = costs[i + 1] - costs[i];
            Trace.Assert(Abs(d) == 1);
            if (d == -1) builder.Append('1');
