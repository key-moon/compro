// detail: https://atcoder.jp/contests/arc026/submissions/11759433
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        var l = nl[1];
        SegmentTree<long> segTree = new SegmentTree<long>(l + 1, long.MaxValue / 2, Min);
        var lrcs = Enumerable.Repeat(0, nl[0]).Select(_ => Console.ReadLine().Split().Select(int.Parse).ToArray()).OrderBy(x => x[0]).ToArray();
        segTree[0] = 0;
        foreach (var lrc in lrcs)
        {
            segTree.Operate(lrc[1], segTree.Query(lrc[0], lrc[1]) + lrc[2]);
        }
        Console.WriteLine(segTree[l]);
    }
}
