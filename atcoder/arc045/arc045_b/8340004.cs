// detail: https://atcoder.jp/contests/arc045/submissions/8340004
using System;
using System.IO;
using System.Linq;
...
public static class P
{
    public static void Main()
    {
        ...
        var sections = Enumerable.Repeat(0, nm[1]).Select(_ => Console.ReadLine().Split().Select(x => int.Parse(x) - 1).ToArray()).ToArray();
        ...
        for (int i = 0; i < sections.Length; i++)
            if (1 < segTree.Query(sections[i][0], sections[i][1]))
                res.Add(i + 1);
