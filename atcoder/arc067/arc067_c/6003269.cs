// detail: https://atcoder.jp/contests/arc067/submissions/6003269
using System;
using System.IO;
using System.Linq;
        ...
        for (int memberCount = a; memberCount <= b; memberCount++)
        {
            ...
            for (int groupCount = c; groupCount <= d; groupCount++)
            {
                ...
                for (int i = table.Length - 1; i >= memberCount * groupCount; i--)
                    newTable[i] += table[i - memberCount * groupCount] * (Factorial(n - i + memberCount * groupCount) / Factorial(n - i)) / duplicate;
            }
            table = newTable;
        }
