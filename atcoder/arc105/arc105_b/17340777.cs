// detail: https://atcoder.jp/contests/arc105/submissions/17340777
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        while (true)
        {
            ...
            var min = a.Min();
            var newa = a.Select(x => x % min).Distinct().ToArray();
            var newMin = newa.Min(x => x == 0 ? int.MaxValue : x);
            for (int i = 0; i < newa.Length; i++)
            {
                if (newa[i] != newMin) newa[i] += min;
            }
            a = newa;
        }
    }
