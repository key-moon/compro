// detail: https://atcoder.jp/contests/donuts-2015/submissions/14841488
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        var dic = h.OrderBy(x => x).Select((item, ind) => new { item, ind }).ToDictionary(x => x.item, x => x.ind);
        var a = h.Select(x => dic[x]).ToArray();
        SegmentTree<int, int> segmentTree = new SegmentTree<int, int>(n, -1, -2, (x, y) => x == -1 ? y : x, (x, y) => y == -2 ? x : y, (x, y) => y == -2 ? x : y);
        int[] res = new int[n];
        int[] highers = new int[n];
        for (int i = 0; i < a.Length; i++)
        {
            var higher = segmentTree.Query(a[i], segmentTree.Size - 1);
            var nearest = segmentTree.Query(0, segmentTree.Size - 1);
            highers[i] = higher == -1 ? 0 : highers[higher] + 1;
            res[i] = nearest == -1 ? 0 : highers[nearest] + 1;
            segmentTree.Update(0, a[i], -1);
            segmentTree.Update(a[i], a[i], i);   
        }
