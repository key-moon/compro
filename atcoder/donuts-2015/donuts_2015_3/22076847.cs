// detail: https://atcoder.jp/contests/donuts-2015/submissions/22076847
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        SegmentTree<int, int> segTree = new SegmentTree<int, int>(1000001, 0, -1, (x, y) => x + y, (x, y) => y == -1 ? x : y, (x, y) => y == -1 ? x : y);
        for (int i = 0; i < n; i++)
        {
            var height = h[i];
            Console.WriteLine(segTree.Query(0, 1000000));
            segTree.Update(0, height, 0);
            segTree[height]++;
        }
