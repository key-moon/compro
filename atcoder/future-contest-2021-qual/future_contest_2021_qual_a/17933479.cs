// detail: https://atcoder.jp/contests/future-contest-2021-qual/submissions/17933479
using System;
...
class Solver
{
    ...
    public unsafe string AnnealOnConstCurve()
    {
        ...
        {
            int begin, end, d;
            if ((i / 4) % 2 == 0) (begin, end, d) = (0, Const.W, 1);
            else (begin, end, d) = (Const.W - 1, -1, -1);
            for (int j = begin; j != end; j += d)
            {
                int nbegin, nend, nd;
                if (j % 2 != 0) (nbegin, nend, nd) = (i, i + 4, 1);
                else (nbegin, nend, nd) = (i + 3, i - 1, -1);
                for (int k = nbegin; k != nend; k += nd)
                {
                    if (Board[k, j] != 0) cur[ind++] = Board[k, j];
                }
            }
        }
        cur[ind] = len - 1;
        ...
        int curCost = 0;
        for (int i = 0; i < len - 1; i++) curCost += distance[(cur[i] << 7) + cur[i + 1]];
        curCost += 100 - 1;
...
static class Const
{
    ...
    public const long TLTick = TL * TickPerSec;
