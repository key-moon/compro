// detail: https://atcoder.jp/contests/intro-heuristics/submissions/14815800
//#define B
#undef DEBUG
using System;
...
public static class P
{
    ...
    public static void Main()
    {
        ...
        Debug(curScore);
        Debug(string.Join(",", curOptimal.GroupBy(x => x).Select(x => x.Count())));
        while (true)
        {
            var solution = GenRandomV3(rng.ULong);
            var score = Score(solution);
            //Debug(score);
            //Debug(string.Join(",", solution.GroupBy(x => x).Select(x => x.Count())));
            if (curScore < score)
            {
                curOptimal = solution;
                curScore = score;
