// detail: https://atcoder.jp/contests/intro-heuristics/submissions/14825896
//#define B
//#undef DEBUG
using System;
...
public static class P
{
    ...
    public static void Main()
    {
        ...
        while (true)
        {
            ...
            var range = (rng.Next() & 7) + 1;
            if (d <= start + range) continue;
            var nextScore = current.Shuffle(start, start + range);
    ...
    static int[] GenInit()
    {
        ...
        for (int i = 0; i < t.Length; i++)
        {
            ...
            {
                int score = s[i][j] + (i - last[j]) * c[j];
                //int score = (i - last[j]) * c[j];
                if (maxScore < score)
                {
                    maxScore = score;
                    maxInd = j;
