// detail: https://atcoder.jp/contests/joi2008yo/submissions/6106300
#include <bits/stdc++.h>
using namespace std;
...
int main() 
{
	...
	for (size_t j = 0; j < C; j++)
	{
		...
		{
			sums[i] += table[columns[j] ^ i];
		}
	}
	uint32_t max = 0;
	for (uint16_t i = 0; i < (1 << (R - 1)); i++) max = std::max(max, sums[i]);
	print(max);
	return 0;
}
