// detail: https://atcoder.jp/contests/kupc2017/submissions/17875571
#include <bits/stdc++.h>
using namespace std;
...
signed main() {
  ...
  bigint hash = 0;
  vector<bigint> bases{1};
  for (int i = 0; i < s.size(); i++){
    var lastbase = bases.back();
    hash += lastbase * (s[i] - 'a' + 1);
    var newbase = lastbase * base;
    bases.emplace_back(newbase);
  }
  ...
  while (hash != 0){
    ...
    //aをできる限り大きくしたい
    hash -= dig;
    while (dig + base <= 26 && bigbase <= hash){
      dig += base;
      hash -= base;
    }
    assert(dig != 0);
