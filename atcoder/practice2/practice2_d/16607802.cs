// detail: https://atcoder.jp/contests/practice2/submissions/16607802
using System;
		...
		where TCap : ICap<TValue, TCap>, IEquatable<TCap>, new()
{
	...
	public struct CapInt : ICap<int, CapInt>
	{
		...
		public bool LessThanEquals(ICap<int, CapInt> value) => Value <= value.Value;
		public bool GreaterThan(ICap<int, CapInt> value) => Value > value.Value;
		public bool GreaterThanEquals(ICap<int, CapInt> value) => Value >= value.Value;
	}
	...
	public struct CapLong : ICap<long, CapLong>
	{
		...
		public bool LessThanEquals(ICap<long, CapLong> value) => Value <= value.Value;
		public bool GreaterThan(ICap<long, CapLong> value) => Value > value.Value;
		public bool GreaterThanEquals(ICap<long, CapLong> value) => Value >= value.Value;
	}
		...
		public TValue Flow(int s, int t, TValue flow_limit)
		{
			...
			TCap Dfs(int v, TCap up)
			{
				...
				for (int i = iter[v]; i < _g[v].Count; i++)
				{
					...
					TCap d = Dfs(e.To, up1.LessThan(up2) ? up1 : up2);
					if (d.IsZero() || d.LessThanEquals(ICap<TValue, TCap>.Zero)) continue;
