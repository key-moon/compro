// detail: https://atcoder.jp/contests/rco-contest-2019-qual/submissions/4249941
//#define topt
using System;
using System.IO;
using System.Linq;
...
class P
{
    ...
    static void Main()
    {
        ...
        InitDist();
        int[] maxA = Enumerable.Range(0, n).ToArray();
        double maxScore = -10000;
        for (int _ = 0; _ < 10; _++)
        {
            var cand = Enumerable.Range(0, n).ToArray();
            for (int i = n - 1; i >= 1; i--)
            {
                var j = Abs(random.Int % (i + 1));
                var tmp = cand[i];
                cand[i] = cand[j];
                cand[j] = tmp;
            }
            var score = CalcScore(cand);
            if (maxScore < score)
            {
                maxScore = score;
                maxA = cand;
        ...
        //if (maxA.Distinct().Count() != n) throw new Exception();
        //Console.WriteLine($"{(DateTime.Now - start).TotalMilliseconds},{maxScore}");
        ...
        while (true)
        {
            double duration = (DateTime.Now - start).TotalMilliseconds / 1960;
            if (duration > 1) break;
            iter++;
            ...
            newa = lastA.Take(x).Concat(lastA.Take(y).Skip(x).Reverse()).Concat(lastA.Skip(y)).ToArray();
    ...
    static double CalcScore(int[] a)
    {
        ...
        for (int i = 0; i < res.Length; i++) score -= (res[i] - ave) * (res[i] - ave);
        return score / 200;
    }
}
