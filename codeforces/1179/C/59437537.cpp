// detail: https://codeforces.com/contest/1179/submission/59437537
#include <bits/stdc++.h>
using namespace std;
...
struct SegmentTree
{
    ...
    inline void thrust(int l, int r) {
        ...
        for (; !(Xor >> i); i--) { eval(l >> i); }
        for (; i; i--) { eval(l >> i); eval(r >> i); }
    }
    ...
    void update(int a, int b, E x) {
        ...
        {
            if ((l & 1) == 1) laz[l] = h(laz[l], x), l++;
            if ((r & 1) == 0) laz[r] = h(laz[r], x), r--;
        }
        recalc(a, b);
    }
    ...
    T query(int a, int b) {
        ...
        for (int l = a, r = b; l <= r; l >>= 1, r >>= 1)
        {
            if ((l & 1) == 1) vl = f(vl, reflect(l++));
            if ((r & 1) == 0) vr = f(reflect(r--), vr);
        }
        return f(vl, vr);
    }
