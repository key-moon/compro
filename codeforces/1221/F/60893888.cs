// detail: https://codeforces.com/contest/1221/submission/60893888
using System;
using System.IO;
using System.Net;
...
public static class P
{
    public static void Main()
    {
        ...
        var points = Enumerable.Repeat(0, n).Select(_ =>
        {
            var xyc = Console.ReadLine().Split().Select(int.Parse).ToArray();
            return new Point() { x = xyc[0], y = xyc[1], c = xyc[2] };
        }).OrderBy(x => Min(x.x, x.y)).ToArray();
        var elems = points.Select(x => x.x).Concat(points.Select(x => x.y)).Distinct().OrderBy(x => x).ToArray();
        var dict = elems.Select((Elem, Ind) => new { Elem, Ind }).ToDictionary(x => x.Elem, x => x.Ind);

        List<Point>[] pointPerInitCoods = Enumerable.Repeat(0, dict.Count).Select(_ => new List<Point>()).ToArray();

        SegmentTree<long, long> segTree =
            new SegmentTree<long, long>(elems.Select(x => (long)-x).ToArray(), long.MinValue, 0, Max, (x, y) => x + y, (x, y) => x + y);

        foreach (var point in points)
        {
            segTree.Update(dict[Max(point.x, point.y)], segTree.Size - 1, point.c);
            pointPerInitCoods[dict[Min(point.x, point.y)]].Add(point);
        }

        long res = segTree.Query(0, segTree.Size - 1) + elems[0];
        int minCood = elems[0];

        for (int i = 0; i < elems.Length - 1; i++)
        {
            foreach (var point in pointPerInitCoods[i])
                segTree.Update(dict[Max(point.x, point.y)], segTree.Size - 1, -point.c);
            var curRes = segTree.Query(i + 1, segTree.Size - 1) + elems[i + 1];
            if (res < curRes)
            {
                res = curRes;
                minCood = elems[i + 1];
        ...
        {
            Console.WriteLine(0);
            Console.WriteLine($"1333333333 1333333333 1333333333 1333333333");
            return;
        }
        Console.WriteLine(res);
        {
            long curScore = 0;
            int maxCood = minCood;
            foreach (var item in points.Where(x => minCood <= Min(x.x, x.y)).OrderBy(x => Max(x.x, x.y)))
            {
                if (curScore == res) break;
                var max = Max(item.x, item.y);
                curScore += item.c;
                curScore -= max - maxCood;
                maxCood = max;
            }
            Console.WriteLine($"{minCood} {minCood} {maxCood} {maxCood}");
        }
    }
}
...
class SegmentTree<DataT, OperatorT> : IEnumerable<DataT>
{
