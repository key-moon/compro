// detail: https://codeforces.com/contest/1299/submission/70634821
using System;
...
public static class P
{
    public static void Main()
    {
        ...
        SegmentTree<int> table = new SegmentTree<int>(a, 0, (x, y) => x | y);
        int max = -1;
        int maxind = -1;
        for (int i = 0; i < a.Length; i++)
        {
            var remain = table.Query(0, i - 1) | table.Query(i + 1, table.Size - 1);
            var res = (a[i] | remain) - remain;
            if (max < res)
            {
                max = res;
                maxind = i;
        ...
        Console.WriteLine($"{a[maxind]} {string.Join(" ", a.Where((_, ind) => ind != maxind))}");
    }
}
