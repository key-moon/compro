// detail: https://codeforces.com/contest/1470/submission/103399971
using System;
...
public static class P
{
    ...
    static void Solve()
    {
        ...
        var sorted = k.Select((elem, ind) => (elem, ind)).OrderBy(_ => rng.Next()).OrderBy(x => x.elem).ToArray();

        long cost = 0;
        Queue<int> queue = new Queue<int>();
        int ptr = 0;
        foreach (var (elem, ind) in sorted)
        {
            queue.Enqueue(c[elem - 1]);
            if (elem <= ptr || queue.Peek() < c[ptr])
            {
